...
| 角の形式の選択 | 角丸の曲率を CSS `corner-shape` で切り替え（標準／なめらか(Mac風)／面取り／えぐり／切り欠き／直角）。`superellipse(k)` による数値指定（-5〜5）にも対応。未対応ブラウザでは「標準」で表示 |
...
| 日英の表示言語切替 | ポップアップとサイドパネルのヘッダーにある `JA \| EN` で UI の言語を切り替え。初回はブラウザの言語から自動判定し、一度切り替えるとその選択を記憶 |
...
2. 「**マークモード**」をオンにする
3. 枠の色・線種・線幅・余白・角丸・角の形式を選ぶ（連番ラベルは既定オフ。「表示」で切り替え）
